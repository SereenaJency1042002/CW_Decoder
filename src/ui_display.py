...
class UIDisplay:
    ...
    def _populate_devices(self):
        ...
        try:
            ...
            for idx, name in input_devices:
                label = f"{name} (Recommended)" if "cable" in name.lower() else name
                names.append(label)
    ...
    def _get_device_index(self, selected_name):
        ...
        try:
            import sounddevice as sd
            devices = sd.query_devices()
            # Strip the recommended suffix we added for VB-Cable
            clean_name = selected_name.replace(" (Recommended)", "").strip()
    ...
    def _build_text_boxes(self):
        ...
        # Top half — offline T5 corrected text
        ctk.CTkLabel(
            self.ai_frame, text=self._spaced("OFFLINE T5"),
        ...
        ctk.CTkFrame(self.ai_frame, fg_color=_BORDER, height=1).grid(row=2, column=0, sticky="ew", pady=(2, 2))

        # Bottom half — Groq corrected text (for side-by-side comparison)
        ctk.CTkLabel(
            self.ai_frame, text=self._spaced("GROQ"),
            font=ctk.CTkFont(size=9), text_color=_MUTED2
        ).grid(row=3, column=0, sticky="nw")
        self.groq_text_box = ctk.CTkTextbox(
            self.ai_frame, fg_color=_BG, text_color=_ORANGE, border_width=0,
            font=ctk.CTkFont(family="Courier New", size=12),
        )
        self.groq_text_box.grid(row=4, column=0, sticky="nsew", pady=(2, 4))
    ...
    def decode(self):
        ...
        def _process():
            ...
            # Run both AI correctors for side-by-side comparison
            from src.offline_ai import OfflineAI
            offline_text = OfflineAI().correct(corrected_text, raw_morse)

            if self.groq_api_key:
                from src.ai_predictor import AIPredictor
                try:
                    groq_text = AIPredictor(self.groq_api_key).correct(corrected_text, raw_morse)
                except Exception:
                    groq_text = "(Groq request failed)"
            else:
                groq_text = "(no GROQ_API_KEY set)"
            ...
            self.app.after(0, lambda: self._on_decode_ready(session, y, sr, filtered, corrected_events, offline_text, groq_text, raw_data, raw_sr))
    ...
    def _on_decode_ready(self, session, y, sr, filtered, events, offline_text="", groq_text="", raw_data=None, raw_sr=None):
        ...
        if offline_text:
            self.app.after(settle_ms, lambda s=session, txt=offline_text: self._set_offline_text(s, txt))
        if groq_text:
